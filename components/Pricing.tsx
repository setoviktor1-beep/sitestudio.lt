      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          {dict.pricing.plans.map((plan) => (
            <div
              key={plan.name}
              className="bg-white p-8 rounded-2xl border border-[#0f172a]/10 shadow-soft relative flex flex-col"
            >
              ...
              <Link
                href={contactHref}
                className="w-full text-center py-3.5 rounded-xl font-semibold text-sm transition-colors bg-[#0f172a] text-white hover:bg-[#2456d6]"
              >
                {plan.cta}
              </Link>
            </div>
