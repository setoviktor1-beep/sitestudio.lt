    ...
    <section id="paslaugos" className="py-24 bg-[#f7f7fb]">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <p className="text-sm font-semibold text-[#2563eb] uppercase tracking-wider mb-3">Paslaugos</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {services.map((service, index) => (
            <div
              key={index}
              className="bg-white p-8 rounded-3xl border border-black/5 card-hover shadow-[0_8px_30px_rgba(20,18,60,0.04)]"
            >
              <div className="flex items-start justify-between mb-5">
                <div className="text-4xl p-3 rounded-2xl bg-[#f7f7fb] border border-black/5">
                  {service.icon}
                </div>
              ...
              <ul className="space-y-2.5 border-t border-black/5 pt-6">
                {service.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-center text-sm text-[#3b3b50]">
                    <span className="mr-2.5 text-[#2563eb] font-bold">✓</span>
                    {feature}
                  </li>
                ))}
              </ul>
