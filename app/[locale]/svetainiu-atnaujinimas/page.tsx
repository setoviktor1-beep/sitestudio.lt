...
export async function generateMetadata({
  ...
  return {
    title: `${dict.services.items[3].title} — SiteStudio`,
    description: dict.services.items[3].description,
    alternates: { canonical: `/${locale}/svetainiu-atnaujinimas`, languages: pathAlternates("/svetainiu-atnaujinimas") },
  };
}

export default async function LocalizedSvetainiuAtnaujinimasPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const crumbs = [
    ...
    { name: dict.services.items[3].title },
  ];

  const jsonLd = siteGraph(
    webPageNode(`/${locale}/svetainiu-atnaujinimas`, `${dict.services.items[3].title} — SiteStudio`, dict.services.items[3].description, locale),
    breadcrumbNode(`/${locale}/svetainiu-atnaujinimas`, crumbs),
  );
