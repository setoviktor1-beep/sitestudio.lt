...
export default async function LocalizedInternetoSistemuPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const crumbs = [
    ...
    { name: dict.services.items[4].title },
  ];

  const jsonLd = siteGraph(
    webPageNode(`/${locale}/interneto-sistemu-kurimas`, `${dict.services.items[4].title} — SiteStudio`, dict.services.items[4].description, locale),
    breadcrumbNode(`/${locale}/interneto-sistemu-kurimas`, crumbs),
  );
