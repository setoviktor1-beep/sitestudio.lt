...
export async function generateMetadata({
  ...
  return {
    title: `${dict.services.items[0].title} — SiteStudio`,
    description: dict.services.items[0].description,
    alternates: { canonical: `/${locale}/svetainiu-kurimas`, languages: pathAlternates("/svetainiu-kurimas") },
  };
}

export default async function LocalizedSvetainiuKurimasPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const jsonLd = siteGraph(
    webPageNode(`/${locale}/svetainiu-kurimas`, `${dict.services.items[0].title} — SiteStudio`, dict.services.items[0].description, locale),
    breadcrumbNode(`/${locale}/svetainiu-kurimas`, crumbs),
  );
