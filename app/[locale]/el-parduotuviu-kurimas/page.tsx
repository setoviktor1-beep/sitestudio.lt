...
export async function generateMetadata({
  ...
  return {
    title: `${dict.services.items[1].title} — SiteStudio`,
    description: dict.services.items[1].description,
    alternates: { canonical: `/${locale}/el-parduotuviu-kurimas`, languages: pathAlternates("/el-parduotuviu-kurimas") },
  };
}

export default async function LocalizedElParduotuviuPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const jsonLd = siteGraph(
    webPageNode(`/${locale}/el-parduotuviu-kurimas`, `${dict.services.items[1].title} — SiteStudio`, dict.services.items[1].description, locale),
    breadcrumbNode(`/${locale}/el-parduotuviu-kurimas`, crumbs),
  );
